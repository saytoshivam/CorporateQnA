...
AS
SELECT [AspNetUsers].[Id],[FullName],[UserImage],[JobLocation],[Designation],[Department], 
COUNT([AskedBy]) AS [QuestionsAsked],[LikedBy],[DisLikedBy],
(
...
ON [Answers].[AnsweredBy]=[AspNetUsers].[Id] GROUP BY [AspNetUsers].[Id],[FullName],[UserImage],
[JobLocation],[Designation],[Department],[LikedBy],[DislikedBy];